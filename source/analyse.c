...
Changes:
   ...
   09-May-1999  RNW  Modified to cater for multiple library levels.
...
p_ma_t p_ma;
{
 ...
 p_md = &p_ma->ma_defn[p_ma->ma_minlev];
     ...
     if (call->mc_npar != p_md->md_npar)
       {
        ...
        sprintf(linet1,"definition at line %lu specifies %lu parameter%s.",
                (ulong) p_md->md_ps.ps_line,
                (ulong) p_md->md_npar,
                p_md->md_npar==1 ? "" : "s");
        lr_mes(&(call->mc_ps),linet1);
        if (p_md->md_npar==0 && call->mc_npar==1)
          {
 ...
 if (p_md->md_isfil)
   ...
   {
    /* Non-file macros. */
    if (ls_len(p_ma->ma_calls)==0 && !p_md->md_iszer)
       lr_err(&p_md->md_ps,
              "This macro is never used (and has no @Z).");

    if (ls_len(p_ma->ma_calls)>1 && !p_md->md_isman)
       lr_err(&p_md->md_ps,
...
/*                                                                            */
/* Although the actions here may seem rather time expensive, they may not     */
...
#define LEVINF 30000
 ...
 for (level=0; level<LEVINF && gotone; level++)
   {
    ...
    while (tb_rea(macro_table,PV dummy_name,PV &p_ma))
      {
       if ((maisdef(p_ma)) && (p_ma->ma_level == LEVINF))
         {
          p_md_t p_md = &p_ma->ma_defn[p_ma->ma_minlev];
          p_bpls_t p_bpls = p_md->md_body;
 ...
 while (tb_rea(macro_table,PV dummy_name,PV &p_ma))
    if (maisdef(p_ma) && p_ma->ma_level==LEVINF)
      {
       p_md_t p_md = &p_ma->ma_defn[p_ma->ma_minlev];
       lr_err(&p_md->md_ps,"This macro has an infinite expansion.");
      }
 ...
 while (TRUE)
   {
    ...
    if (p_dc->dc_kind==DC_TYPE &&
        p_dc->dc_pty->ty_kind==TY_NSEC &&
        !p_dc->dc_pty->ty_isnam)
      {
