...
Changes:
   ...
   08-May-1999  RNW  Added +u option for HTML documentation.
...
#define ECHO_CL {if (!seen_error)\
 ...
 while (TRUE)
   {/* This loop processes one command line parameter (option) per iteration. */
    ...
    /* As COMLINE_MAX can be greater than PATHNAME_MAX, we have to be careful */
    /* that the user is not about to blow our filename buffer.                */
    if (strlen(cl_temp) > PATHNAME_MAX)
      {
