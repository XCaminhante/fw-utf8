...
#define NEXTUNICH {ch= *++p_ch;} //since UTF-8 is backwards compatible with ASCII, this'll do

#define NEXTCH {if (ch<' ' && ch>=0) {if (ch==EOL) prepline(p_ch+1);} else NEXTUNICH;}
...
p_ps_t p_ps;
{
 ...
 {/* This construct does the work of the include.                 */
  ...
  p_filename=mm_temp((size_t) PATHNAME_MAX+1+10); /* 10 is for paranoia. */
  p_tempname=mm_temp((size_t) PATHNAME_MAX+1+10); /* 10 is for paranoia. */
...
p_pt_t   arg;
{
 ...
 /* Check the third argument. */
   ...
   {
    lr_err(&arg[3].pt_ps,"Expecting one of [none,tex,html].");
    goto help;
   }
 ...
 help:
    lr_mes(&arg[0].pt_ps,
           "The correct format is: \"@p typesetter = none|tex|html\".");
    lr_mes(&arg[0].pt_ps,"Pragma ignored.");
    return;
}
 ...
 switch (toupper(ch))
   {
    ...
    case 'L': sendspec(&ps_spec,TK_LIBR,DONTCARE); break;
 ...
 if (length==0)
   {
    ...
    if (inclevel==0)
      ...
      {
       /* 08-May-1999 RNW Include file name added.   */
       /* See 8 May note at the end of the function. */
       ps.ps_line=globalno;
       if (option.op_b7_b)
          sprintf(linet1,"Include file \"%s\" is empty (not a byte in syte)!",SUPPNAME);
       else
          sprintf(linet1,"Include file \"%s\" is empty (not a byte in syte)!",p_fname);
       lr_war(&ps,linet1);
      }
    return;
   }
 ...
 if (addedeol)
   {
    ...
    if (inclevel==0)
      ...
      {
       /* 08-May-1999 RNW FunnelWeb 3.0 didn't give the name of the included  */
       /* file when saying that an included file was not properly terminated. */
       /* This caused significant confusion when there was a list of          */
       /* inclusions, as FunnelWeb's error was seemingly pointing to the NEXT */
       /* inclusion! So I've added code to include the filename in the error  */
       /* message so as to eliminate the possibility of such confusion.       */
       if (option.op_b7_b)
          sprintf(linet1,"The last line of the include file \"%s\" was terminated by EOF.",SUPPNAME);
       else
          sprintf(linet1,"The last line of the include file \"%s\" was terminated by EOF.",p_fname);
       lr_war(&ps,linet1);
      }
